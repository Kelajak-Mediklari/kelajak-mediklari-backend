...
class PaymentCallbackView(APIView):
    ...
    def _process_successful_payment(self, transaction):
        ...
        # Convert free trial UserCourse to paid or create new paid UserCourse
        from apps.course.models import UserCourse

        user_course = UserCourse.objects.filter(
            user=transaction.user, course=transaction.course, is_free_trial=True
        ).first()

        if user_course:
            # User had free trial, convert it to paid access
            user_course.is_free_trial = False
            user_course.save()
        else:
            # No free trial existed, create new paid UserCourse
    ...
    def _process_failed_payment(self, transaction):
        ...
        CoinReservation.objects.filter(transaction=transaction, is_active=True).update(
            is_active=False
        )
